 ...
 * @param app
 */
async function runHttps(app: express.Express) {
  if (process.env['IDENTITY_PATH'] && process.env['TRUST_PATH']) {
    try {
      const identityValue = await fs.readFile(
          process.env['IDENTITY_PATH'] as string, {encoding: 'utf8'});
      const trustValue = await fs.readFile(
          process.env['TRUST_PATH'] as string, {encoding: 'utf8'});
